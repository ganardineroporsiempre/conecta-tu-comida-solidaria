  ...
  return (
    <nav className="bg-white shadow-sm sticky top-0 z-50">
      <div className="conecta-container py-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <a href="#" className="flex items-center space-x-2">
              <span className="text-conecta-green-500 text-2xl">🍽️</span>
            ...
            <div className="hidden md:flex items-center space-x-8">
              {navLinks.map((link) => (
                <a
                  key={link.name}
                  href={link.href}
                  className="text-gray-600 hover:text-conecta-green-500 font-medium transition-colors"
                >
                  {link.name}
                </a>
              ))}
          ...
          <div className="flex items-center space-x-2">
            ...
            {!isMobile && (
              <Button className="btn-conecta-primary px-4 py-2 ml-4">
                Acceder
              </Button>
            )}
          </div>
        ...
        {/* Mobile Menu */}
        {isMobile && isMenuOpen && (
          <div className="mt-3 pb-3 border-t pt-3 space-y-3 md:hidden animate-fade-in">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="block px-3 py-2 text-gray-600 hover:text-conecta-green-500 hover:bg-gray-50 rounded-md font-medium transition-colors"
